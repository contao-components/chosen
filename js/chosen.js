...
var Chosen = new Class({
    ...
    choice_build: function(item) {
        ...
        document.id(choice_id).getElement("a").addEvent('click', this.choice_destroy_link_click.bind(this));
    },
    ...
    winnow_results: function() {
        var found, parts, regex, result_id, results, searchText, text;
    ...
    search_field_scale: function() {
        var dd_top, div, style_block, styles, w;
        ...
        if (this.is_multiple) {
            ...
            div = new Element('div', {'styles': style_block});
...
Element.implement({
    get_side_border_padding: function() {
        ...
        var mapped = Object.map(notNull, function(s) {
            return s.toInt();
        });
    ...
    add_option: function(option, group_position, group_disabled) {
        if (option.nodeName.toUpperCase() === "OPTION") {
            if (option.text !== "") {
                ...
                if (idx != -1) {
                    opt = opt.substr(0, idx) + '<span class="label-info">' + opt.substr(idx, opt.length) + '</span>';
                }
